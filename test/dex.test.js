...
describe("DexV1", () => {
  ...
  it("Should not execute with 0 ETH", async () => {
    await expectRevert(
      dex.connect(user).makeSwap(ALBT, amountToBuy, {value: 0}),
      "Not enough ETH"
    )
  })
  ...
  it("Should swap only 1 token", async () => {
    ...
    await dex.connect(user).makeSwap(ALBT, amountToBuy, {value: web3.utils.toWei(etherAmount)});
  ...
  it("Should swap several tokens", async () => {
    ...
    await dex.connect(user).makeMultiSwap(toToken, porcToken, amounts, {value: web3.utils.toWei(etherAmount)});
